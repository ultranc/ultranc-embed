...
| [`options.html`](embed/options.html) | Square corners and light mode together |

## Options

The embed takes optional URL parameters. They can be combined, and with no
parameters you always get the standard viewer, in this release and every future
one.

| Parameter | Effect |
|---|---|
| `?squarecorners=default` | Square corners throughout the viewer. Drop the `border-radius` from the iframe style so the frame matches. |
| `?lightmode=default` | The toolpath renders on a light gray background. Panels keep the standard dark look. |

Both together: `?squarecorners=default&lightmode=default`

More on the embed, including a live preview and a snippet generator with these
options: **https://ultranc.com/embed/**
...
|---|---|
| [`tombstone.nc`](samples/tombstone.nc) | **Subprograms**: one 12-hole G81 pattern called under G54 to G57 via `M98`, giving 48 holes from a 6-line main program, plus a `G91 G28 Z0` reference return |
...
| [`dwell.nc`](samples/dwell.nc) | `G04` dwells (P/X words, timing depends on dialect) |
...
Why path divided by feed underestimates, what subprograms do to the clock, G84 tapping,
...
is free to use and free to embed, but its source is not open, and this is the
